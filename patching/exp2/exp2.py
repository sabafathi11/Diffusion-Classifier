...
def eval_error(unet, scheduler, latent, all_noise, ts, noise_idxs,
    ...
    with torch.inference_mode():
        for _ in tqdm.trange(len(ts) // batch_size + int(len(ts) % batch_size != 0), leave=False):
            ...
            if loss == 'l2':
                error_tensor = F.mse_loss(noise, noise_pred, reduction='none')
            elif loss == 'l1':
                error_tensor = F.l1_loss(noise, noise_pred, reduction='none')
            elif loss == 'huber':
                error_tensor = F.huber_loss(noise, noise_pred, reduction='none')
            else:
                raise NotImplementedError

            # Print full tensor before reduction
            for idx_in_batch in range(error_tensor.size(0)):
                torch.save(error_tensor[idx_in_batch],f"error{idx_in_batch}.pt")

            # Reduce to scalar for each sample
            error = error_tensor.mean(dim=(1, 2, 3))
...
def main():
    ...
    idxs_to_eval = idxs[args.worker_idx::args.n_workers][:5]
